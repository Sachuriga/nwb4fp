...
def border_score(rate_map, fields):
    raise DeprecationWarning('This function is moved to "nwb4fp.analyses.border_scores"')
...
def remove_fields_by_area(fields, minimum_field_area):
    ...
    # Create a dictionary of labels and counts, excluding background (0)
    label_counts = {lab: count for lab, count in zip(labels, counts) if lab != 0}
    
    # Filter out labels with counts below minimum_field_area
    valid_labels = {lab: count for lab, count in label_counts.items() 
                   if count >= minimum_field_area}
    
    if not valid_labels:  # If no labels remain after filtering
        fields[fields != 0] = 0
        return fields
    
    # Sort labels by count in descending order
    sorted_labels = sorted(valid_labels.items(), key=lambda x: x[1], reverse=True)
    
    # Create mapping from old labels to new labels (starting from 1)
    label_map = {old_label: new_label + 1 
                for new_label, (old_label, _) in enumerate(sorted_labels)}
    
    # Create output array
    result = np.zeros_like(fields)
    
    # Remap the valid labels
    for old_label, new_label in label_map.items():
        result[fields == old_label] = new_label
